        ...
        (set) => ({
            ...
            incrementInitiative: () => {
                set((state) => {
                    ...
                    const newInitiative = state.initiative.map(
                        (creature, i) => {
                            if (i === newActiveIndex) {
                                return {
                                    ...creature,
                ...
                set((state) => {
                    const newInitiative = state.initiative.map(
                        (creature, i) => {
                            if (i === index) {
                                if (
                                    creature?.immunities.includes(damage.type)
                                ) {
                                    return creature;
                                }
                                if (
                                    creature?.resistances.includes(damage.type)
                                ) {
                                    damage.amount = Math.floor(
                                        damage.amount / 2
                                    );
                                }
                                ...
                                if (creature.tempHp > 0) {
                                    ...
                                    return {
                                        ...creature,
                                        tempHp: creature.tempHp - damage.amount,
                                    };
                                }
            ...
            removeCreature: (index: number) => {
                set((state) => {
                    const newInitiative = state.initiative.filter(
                        (_, i) => i !== index
                    );
                    return { initiative: newInitiative };
                });
