...
export const EditCreatureForm: React.FC = () => {
  ...
  const index = editIndex !== activeIndex ? editIndex : activeIndex;

  const creature = initiative[index];
  ...
  const handleInitiative: SubmitHandler<EditCreatureFormType> = (data) => {
    setCreatureInitiative(index, data.initiative);
  };

  const handleTempHp: SubmitHandler<EditCreatureFormType> = (data) => {
    setCreatureTempHp(index, data.tempHp);
  };
  ...
  const onSubmitHeal: SubmitHandler<EditCreatureFormType> = (data) => {
    ...
    creatureHeal(index, data.amount);
  };
  ...
  const onSubmitDamage: SubmitHandler<EditCreatureFormType> = (data) => {
    ...
    creatureDamage(index, data);
  };

  return (
    <>
      <div className="p-3 hidden md:block">
        <CreatureCard creature={creature} index={index} sidebar={true} />
      </div>
      <FormProvider {...methods}>
        <Form className="@container p-3 gap-4 flex flex-col">
          ...
          <span className="flex-col w-28 flex gap-0 @xs:gap-2 mb-4">
            <Button
              type="submit"
